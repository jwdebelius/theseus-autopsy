...
def build_filter_fits(example_data, filters, models, tech_order):
    """
...
def build_per_cohort_fit(example_data):
    ...
    example_data: pd.DataFrame
        Simulated data with columns smoking, race, ethncity, sex, m_age, 
        year, the simulated birthweight in y
...
def plot_weight_dist(example_data, ax1, palette, box_kws=None, 
                     violin_kws=None):
    """
...
def plot_pooled_violin(axes, example_data, box_kws=None, violin_kws=None):
    ...
    for i, (ax_dis, vals, title) in enumerate(zip(*(value_groups))):
        ...
        sn.boxplot(x='smoking', hue='smoking', y='y', data=sub_data, ax=ax_dis,
                   **boxplot_kwargs)
...
def plot_table_1(ax, example_table, fontsize=8):
    """
    Formats table 1 as a latex table for plotting
    """
    exp_t1_tex = _table1_to_latex(example_table).replace('\n', ' ')
...
def summarize_simulated_cohorts(fig, data):
    """
    ...
    _plot_cohort_summary(ax0, data)
    ax0.set_xlim(0, max(ax0.get_xlim()[1], 500))
    ax0.set_ylim(-1, data['x'].max() + 1)
    ...
    for col_, axi in cat_pos.items():
        ...
        _plot_categorical_by_cohort(ax, data, col_, colors=palettes[col_])
        ax.set_xlim(0, 1)
    ...
    for axi, (col, xlim) in enumerate(con_pos.items()):
        ...
        _plot_cont_histogram_by_cohort(ax, data, col, xlim=xlim)
    ...
    _plot_cont_kde_by_cohort(ax, data, 'y')
    ax.xaxis.set_tick_params(bottom=True, labelbottom=True, labelsize=8)
...
def tabulate_example_data(example_data, levels=['cohort_type', 'param', 'mean'], 
                          palette=None):
    """
    Combines the example data fits and calculates the x position
    """
    example_summary = pd.concat(
        axis=1, 
        objs=[build_per_cohort_fit(example_data), 
              _extract_example_data_summary(example_data), 
              _summarize_example_smoking(example_data)]
    )
    ...
    palettes = sn.color_palette(palette, n_colors=2)[::-1]
    
    palettes = {i: mpc.to_hex(c) for i, c in enumerate(palettes)}
    example_summary['color'] = \
        example_summary['cohort_type'].replace(palettes)
...
def tabulate_res(fit_res, filter_order, tech_order, palette='Dark2'):
    ...
    idx_ = idx_.replace(
        to_replace={
            'model': {id_: str(i * (len(filter_order) + 2.5))
                      for i, id_ in enumerate(tech_order)},
            'filter': {id_: str(i) for i, id_ in enumerate(filter_order)},
        },
        ).astype(float)
...
def _extract_example_data_summary(example_data):
    """
...
def _summarize_example_smoking(example_data):
    """
...
def _table1_to_latex(example_table_1):
    """
    ...
    replacements = {
        '\\multirow[t]{2}{*}{sex}': 'sex',
        '\\multirow[t]{4}{*}{race}': 'race',
        '\\multirow[t]{2}{*}{race}': 'race',
        '\\multirow[t]{3}{*}{race}': 'race',
        '\\multirow[t]{2}{*}{ethnicity}': 'ethnicity',
        'sample_size': '', 
        'm_age & mean (std)': '\\multicolumn{2}{l}{mat. age, mean (std)}',
    ...
    exp_t1_tex = exp_t1_tex.replace('_', '\_')
    exp_t1_tex = exp_t1_tex.replace('%', '\%')
