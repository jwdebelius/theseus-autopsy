...
class SimulationSummary():
    ...
    def tabulate_results(self):
        ...
        """
        if levels is None:
            levels = self._y_sort_levels
        if ascending is None:
            ascending = self._y_sort_ascend
        ...
        summary.sort_levels(levels, ascending=ascending, inplace=True)
    ...
    def build_filter_fits(self, models, tech_order):
           """
        ...
        DataFrame
        """
        example_filtered = {f_name: f_(self.example_data) 
    ...
    def _summarize_example_smoking(self):
        ...
        smoke_counts = some_counts.count()
...
class ExampleScale:
    def __int__(params, pass_thresh=2, lim_scale=0.85):
    ...
    def _calculate_scale(self):
        ...
        example_params = params.copy().dropna()
        ...
        left_, lo_lims, main_lims = self._check_params(*low_kwargs)
    ...
    @staticmethod
    def _check_params(check_param, main_lims, nom_scaler, pass_bse_q):
        """
...
def plot_per_cohort_data(sim_summary, effect, fig=None, palette=None, pass_thresh=2, lim_scale=0.85):
    ...
    left_, right_, main_lims, lo_lims, hi_lims = \
        ExampleScale(sim_summary.__build_per_cohort_fit(),
...
def plot_cohort_demo_summary(sim_summary, fig=None):
    ...
    for col_, axi in cat_pos.items():
        ...
        _plot_categorical_by_cohort(ax, im_summary.data, col_, 
    ...
    _plot_cont_kde_by_cohort(ax, data, 'y', palette=sim_summary.type_palette)
    ax.xaxis.set_tick_params(bottom=True, labelbottom=True, labelsize=8)
